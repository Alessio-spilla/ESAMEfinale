...
class DAO():
    #SEMPRE PROVARE LA CONNESSIONE

    @staticmethod
    def getAllNodes(ch_min, ch_max):
        ...
        if cnx is None:
            ...
            query = """SELECT g.*, c.Localization as Localization
                        FROM genes g, classification c
                        where g.Chromosome between %s and %s
                        and g.GeneID = c.GeneID
                        """
...
for row in cursor:
    ...
    results.append((row["product_id"], row["product_name"], quantity))
